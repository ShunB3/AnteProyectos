...
<head>
    ...
    <title>Interfaz de Evaluador</title>
    <style>
        /* Estilos generales */
        body {
...
<body>
    <form action="" method="post" style="margin-top: 50px;">
        <h2>Iniciar Sesión como Evaluador</h2>
    ...
    <c:if test="${param.modifica != null}">
        <sql:query var="result" dataSource="${proyecto}">
            SELECT p.correo, p.contrasena FROM profesor p WHERE p.correo = ? AND p.contrasena = ? AND p.ced IN (SELECT i.id_evaluador FROM ideasproyecto i);
